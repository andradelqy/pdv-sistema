...
export function EntregasPDV() {
  ...
  // Carrinho
  const [carrinho, setCarrinho] = useState<{ produtoId: string; quantidade: number }[]>([])
  ...
  // Auto preencher dados se selecionar cliente existente
  ...
  // Geocodificação / Estimativa automática de distância com base no endereço digitado
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          // Coordenada base padrão da loja
          const latLoja = -23.4545
          const lonLoja = -46.5341

          // Distância em linha reta (Haversine) * fator 1.35 de malha viária urbana
          const R = 6371
          ...
          // Heurística de fallback aproximada
  ...
  // Cálculo automático do frete com base na distância e no valor dos produtos
  useEffect(() => {
    const km = distanciaKm
    const freteFixoBase = 6.00 // Fixo R$ 6,00 para distâncias curtas (até 3 km)
    ...
    // Regras de valor do produto / bonificação
    if (subtotal >= 140) {
  ...
  const criarPedidoEntrega = () => {
    ...
    // Notifica canal realtime para o App do Entregador
    try {
  ...
  // Lista de entregas abertas/recentes
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Grid Principal: Produtos + Dados de Entrega */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="overflow-y-auto max-h-72 flex flex-col gap-1">
            {produtosFiltrados.slice(0, 20).map(p => (
              <button
                ...
                className="flex items-center justify-between p-2.5 rounded-lg border border-border text-left hover:bg-muted transition"
              >
                <div>
            ...
            <div className="max-h-40 overflow-y-auto flex flex-col gap-1.5">
              {carrinho.length === 0 ? (
                ...
                carrinho.map(item => {
                  ...
                  if (!p) return null
                  return (
                    <div key={item.produtoId} className="flex items-center justify-between gap-2 p-1.5 rounded-lg bg-muted/40 text-xs">
                      <span className="truncate flex-1 font-medium">{p.nome}</span>
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => updateQty(item.produtoId, item.quantidade - 1)} className="p-1 hover:bg-muted rounded"><Minus size={12} /></button>
                        <span className="font-bold">{item.quantidade}</span>
                        <button onClick={() => updateQty(item.produtoId, item.quantidade + 1)} className="p-1 hover:bg-muted rounded"><Plus size={12} /></button>
                      </div>
                      <span className="w-16 text-right font-semibold">{fmtR(p.precoVenda * item.quantidade)}</span>
                      <button onClick={() => removeItem(item.produtoId)} className="text-rose-500 hover:opacity-70"><Trash2 size={13} /></button>
                    </div>
                  )
                })
              )}
        ...
        {/* Coluna 2: Dados de Entrega e Pagamento */}
        <div className="card-adega p-4 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            <div className="sm:col-span-2">
              ...
              >
                ...
                {clientes.map(c => <option key={c.id} value={c.id}>{c.nome} ({c.telefone || 'Sem tel'})</option>)}
              </select>
            </div>
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                {calculandoFrete && (
                  <span className="text-[11px] text-amber-500 flex items-center gap-1">
                    <Loader2 size={11} className="animate-spin" /> Calculando distância...
                  </span>
                )}
              </div>
              <input
            ...
            {/* Informação do Frete Calculado Automaticamente */}
            <div className="sm:col-span-2 p-3 bg-muted/40 rounded-xl border border-border flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 font-semibold text-slate-800 dark:text-white text-xs">
                  <Sparkles size={14} className="text-amber-500" /> Frete Calculado Automaticamente
                </div>
            ...
            <div>
              ...
              <input
                type="number"
                step="0.50"
                className="w-full p-2 border border-border rounded-lg bg-background font-medium"
                value={taxaEntrega}
            ...
            <div className="sm:col-span-2">
              <label className="font-semibold text-muted-foreground uppercase block mb-1">Observações (Ponto de Ref / Troco)</label>
              <input
                className="w-full p-2 border border-border rounded-lg bg-background"
          ...
          <div className="border-t border-border pt-3 mt-auto">
            <div className="flex justify-between text-xs text-muted-foreground mb-1">
              <span>Subtotal: {fmtR(subtotal)}</span>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-2.5 rounded-lg border border-border text-xs font-semibold hover:bg-muted"
              >
                Limpar
              </button>
              <button
                onClick={criarPedidoEntrega}
                className="flex-1 py-2.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm flex items-center justify-center gap-1.5 shadow-md shadow-amber-500/20 active:scale-95 transition"
              >
                    ...
                    <td className="flex items-center gap-1">
                      {e.status !== 'em_rota' && (
                        <button
                          ...
                          className="px-2 py-1 bg-amber-500 hover:bg-amber-600 text-white text-xs rounded font-medium"
                          title="Mudar status para em rota"
                        >
                          Despachar
                        </button>
                      )}
                      <button
                        ...
                        className="px-2 py-1 bg-emerald-600 hover:bg-emerald-700 text-white text-xs rounded font-medium flex items-center gap-1"
                        title="Marcar como entregue e liquidar"
                      >
