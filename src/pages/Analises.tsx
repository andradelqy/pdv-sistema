...
export function Analises({ tipo }: { tipo: 'abc' | 'qpr' | 'alertas' | 'caixa' | 'historico' | 'backup' }) {
  const { produtos, vendas, clientes, caixaEntradas, caixas, caixaAberto, entregas, movimentacoes, abrirCaixa, fecharCaixa, resetDemo, clearAll } = useStore()
  ...
  if (tipo === 'alertas') {
    ...
    return <div className="flex flex-col gap-4"><h2 className="text-xl font-bold">Alertas</h2>{itens.length ? <div className="flex flex-col gap-2">{itens.map((a, i) => <div key={i} className="card-adega p-4 flex gap-3 items-center"><span className={`badge-adega ${a[2]}`}>{a[0]}</span><span className="text-sm">{a[1]}</span></div>)}</div> : <div className="card-adega p-8 text-center text-success">Tudo em ordem.</div>}</div>
  }
