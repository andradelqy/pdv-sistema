-- Corrige o registro da auditoria sem conceder INSERT direto na tabela.
-- A função pública permanece SECURITY INVOKER; somente este helper interno,
-- com autorização explícita, ultrapassa o RLS da trilha imutável.
begin;
...
begin
  ...
  if p_venda_id is null or length(trim(p_venda_id)) = 0 or p_total <= 0 or p_itens <= 0 then
  ...
  values
    (v_loja, v_usuario, 'confirmou', 'venda', p_venda_id,
